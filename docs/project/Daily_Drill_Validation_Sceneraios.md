...
graph TB
    ...
    subgraph "Database"
        ...
        DB2[(user_usage_tracking<br/>+ bonus_count)]
        DB3[(user_subscriptions<br/>+ trial cols)]
    end
...
-- 6. Reset trial for re-testing (DEV ONLY)
...
```dart
// Feature keys — match features table in DB
...
// Usage limit rule keys — match usage_limit_rules.rule_key
UsageLimitKeys.mockInterviews      // 'mock.interviews'       weekly
UsageLimitKeys.mockQuestions       // 'mock.questions'        weekly
UsageLimitKeys.dailyDrillQuestions // 'daily_drill.questions' daily

// accessStateProvider API (reactive via ref.watch)
access.isFree                             // tier == 'FREE'
access.isPaidUser                         // PRO or ELITE
access.isTrialActive                      // 3-day trial running
access.canAccess(FeatureKeys.xyz)         // feature gate (boolean)
...
> [!NOTE]
> `_InterviewQuotaChip` and `DailyDrillScreen` both use `ref.watch(accessStateProvider)` — they live-update automatically when `accessProvider.notifier.refreshUsage()` is called after any EF write.

> [!IMPORTANT]
> **Trial features unlock** — `activate_free_trial()` RPC only sets `is_trial_active = true`. Feature unlocking during the trial depends on `resolve-user-access` EF reading that flag and adding trial-tier feature keys to `features_enabled`. Verify this branch exists in the EF if trial features don't unlock.

> [!WARNING]
> **Ad simulation in production** — `watchAdForBonus()` currently uses `Future.delayed(5s)` to simulate a rewarded ad. Replace with your real Ad SDK call before release. The `grant-drill-ad-bonus` EF currently trusts the client — add server-side ad verification tokens to prevent abuse.
